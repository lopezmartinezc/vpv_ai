...
from src.features.draft_assistant.service import _clean_setting


def test_strips_a_trailing_comment() -> None:
    assert _clean_setting("anthropic          # anthropic | openai") == "anthropic"


def test_strips_surrounding_whitespace() -> None:
    assert _clean_setting("  openai \n") == "openai"


def test_leaves_a_clean_value_alone() -> None:
    assert _clean_setting("claude-opus-5") == "claude-opus-5"


def test_handles_missing_value() -> None:
    assert _clean_setting(None) == ""
    assert _clean_setting("") == ""
